...
hbar = 1.0
L =6.0                                  # spatial grid half-length
...
def wigner_p0(psi, dx):
    ...
    return W/np.max(np.abs(W))  # normalize to 1
...
def fit_gaussian(x, y):
    ...
    p0 = [x[np.argmax(y_norm)], np.std(x)]  # initial guess: [center, width]
    try:
        popt, _ = curve_fit(gaussian, x, y_norm, p0=p0)
        x0, sigma = popt
    except RuntimeError:
        # fallback if fit fails
        x0 = x[np.argmax(y_norm)]
        sigma = np.sqrt(np.sum(y*(x-x0)**2)/np.sum(y))
    W_fit = gaussian(x, x0, sigma)
    return W_fit, x0, sigma
...
with open(gauss_file, "w") as gf:
    gf.write("# vib   a   sigma; (Gaussian fit: W(x) = exp(-(x-a)^2 / (2*sigma^2))) \n")
...
for m in modes:
    ...
    W_fit, x0, sigma = fit_gaussian(x_wig, W_anh)
    ...
    plt.plot(xgrid, psi_abs, 'o-', label='|Psi0(x)|')
    ...
    plt.plot(xgrid, W_fit, '-', label='Gaussian Fit')
    plt.xlabel("x (Angstroms)")
    plt.ylabel("W(x, P=0)")
